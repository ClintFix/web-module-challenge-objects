...
*/
const item1 = createMenuItem("pizza",5,"lunch");
const item2 = createMenuItem("calzone",7,"dinner");
const item3 = createMenuItem("soup",3,"dinner");
...
export const burger = {
  ...
  price: 18, 
  category: "Lunch", 
  discount: function(discountGroup) {
    if (discountGroup === 'teacher' || discountGroup === 'student') {
      return this.price * .75;
    }
    else if (discountGroup === 'public') {
      return this.price * .9;
    }
  }
}
...
*/
reviews.push({name: "Clint", rating: 4, feedback: "Sweet place. Food was great, but a bit pricey."});
console.log(reviews);
...
reviews[7]['feedback'] = "this place is chill with really cool people, grat for getting work done on weekdays";
console.log(reviews);
...
function getReviewByIndex(array, index) {
  const chosenArray = array[index];
  return `${chosenArray["name"]} gave the restaurant a ${chosenArray["rating"]} star review, and their feedback was: ${chosenArray["feedback"]}`
}
...
function getLastReview(array) {
  ...
  return `${array[lastItem]["name"]} gave the restaurant a ${array[lastItem]["rating"]} star review, and their feedback was: ${array[lastItem]["feedback"]}`
} 
...
*/
 ...
 function getReviewByRating(array, desiredRating) {
  const filteredReviews = [];  
  for(let i = 0; i < array.length; i++) {
    if (array[i]["rating"] >= desiredRating && array[i]["rating"] < desiredRating +1) {
